...
![图片](images/11/11-01.png)

![图片](images/11/11-02.png)

使用者浏览页面，分别先后触发了`alert`弹框和`localStorage`获取本地数据：

![图片](images/11/11-03.png)

![图片](images/11/11-04.png)
...
这是因为，一但通过`userProvidedStyles`，恶意用户仍可以提供 `CSS` 来进行“点击诈骗”，例如将链接的样式设置为一个透明的方框覆盖在“登录”按钮之上。然后再把`[https://user-XSS-website.com/](https://user-XSS-website.com/)` 做成你的应用的登录页的样子，它们就可能获取一个用户真实的登录信息，所以Vue推荐使用`对象语法`且只允许用户提供特定的可以安全控制的`property`的值:
...
- [b.com](http://b.com) 向 [a.com](http://a.com) 发送了一个请求：[a.com/act=xx](http://a.com/act=xx)。浏览器会默认携带[a.com](http://a.com)的Cookie

- [a.com](http://a.com)接收到请求后，对请求进行验证，并确认是受害者的凭证，误以为是受害者自己发送的请求

- [a.com](http://a.com)以受害者的名义执行了act=xx
...
![图片](images/11/11-14.png)

一次完整的握手连接如下图:

![图片](images/11/11-15.png)

一旦服务器端返回 101 响应，即可完成 WebSocket 协议切换。服务器端可以基于相同端口，将通信协议从 [http://或](http://xn--1nu/) https:// 切换到 ws://或 wss://。协议切换完成后，浏览器和服务器端可以使用 WebSocket API 互相发送和收取文本和二进制消息。
...
举例来说，用户登陆了银行网站[your-bank.com](http://your-bank.com)，银行服务器发来了一个 Cookie。
...
用户后来又访问了恶意网站`[malicious.com](http://malicious.com)`，上面有一个表单。
...
a.利用已通过认证的用户权限更新设定信息等；

b.利用已通过认证的用户权限购买商品；

c.利用已通过的用户权限在留言板上发表言论。

a.验证码；强制用户必须与应用进行交互，才能完成最终请求。

b.尽量使用 post ，限制 get 使用；get 太容易被拿来做 csrf 攻击；

c.请求来源限制，此种方法成本最低，但是并不能保证 100% 有效，因为服务器并不是什么时候都能取到 Referer，而且低版本的浏览器存在伪造 Referer 的风险。

d.token 验证 CSRF 防御机制是公认最合适的方案。

a.第一步：后端随机产生一个 token，把这个token 保存到 session 状态中；同时后端把这个token 交给前端页面；

b.第二步：前端页面提交请求时，把 token 加入到请求数据或者头信息中，一起传给后端；

c.后端验证前端传来的 token 与 session 是否一致，一致则合法，否则是非法请求。
...
防御X-FRAME-OPTIONS；X-FRAME-OPTIONS HTTP 响应头是用来给浏览器指示允许一个页面可否在<frame>,<iframe> 或者 <object> 中展现的标记。网站可以使用此功能，来确保自己网站内容没有被嵌到别人的网站中去，也从而避免点击劫持的攻击。有三个值：
